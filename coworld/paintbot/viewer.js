  ...
  Module.paintbotState = (data) => {
    ...
    for (let s = 0; s < 2; s++) {
      ...
      scoreLine.title = glory !== null
        ? `Glory ${glory} · ${owned} hearts held. Glory is a self-imposed handicap: it starts at the match length in seconds and loses one per second; thirty seconds without supplies adds 10${data.rulesVersion >= 38 ? ", each glory heart picked up 20, every five seconds behind in lives 1 per life trailed" : ", friendly fire taken in the opening thirty seconds 30 per hit"}. Only the winner keeps it.`
