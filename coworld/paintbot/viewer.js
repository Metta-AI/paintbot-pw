...
(() => {
  ...
  function updateGloryToast(w, rulesVersion) {
    ...
    if (!toast) return;
    const events = rulesVersion >= 37 ? (w.gloryEvents || []).filter(e => w.tick - e.tick < GLORY_TOAST_TICKS) : [];
  ...
  Module.paintbotState = (data) => {
    ...
    for (let s = 0; s < 2; s++) {
      ...
      const glory = data.rulesVersion >= 37 ? (w.glory?.[s] ?? 0) : null;
      ...
      scoreLine.title = glory !== null
        ? `Glory ${glory} · ${owned} hearts held. Glory is a self-imposed handicap: it starts at the match length in seconds and loses one per second; thirty seconds without supplies adds 10${data.rulesVersion >= 39 ? ", each glory heart picked up 20, every five seconds behind in lives 1 per life trailed" : data.rulesVersion >= 38 ? ", each glory heart picked up 20, friendly fire taken in the opening thirty seconds 30 per hit" : ", friendly fire taken in the opening thirty seconds 30 per hit"}. Only the winner keeps it.`
