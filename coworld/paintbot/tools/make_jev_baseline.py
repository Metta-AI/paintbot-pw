...
The layer keeps what those ablations showed works and drops the rest:
- Jev chooses among code-ranked options with consequences in the text: the squad objective
  (capture candidates, a guard option, the big heart, a strike on an enemy heart, or keep
  current) and, for a cog in danger, where to retreat to (keep going, medkit, squadmates, our heart).
- One asker per squad (seat 0, then the next seat if it goes quiet) relays the objective by
  shout, so squads keep agreeing; any cog may ask for itself when it is hurt or outnumbered.
- The survival estimate moves the existing break-off threshold instead of overriding movement.
