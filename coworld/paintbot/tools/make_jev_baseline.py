...
TERRAIN_FIELDS = [
    ("nearest_trench_m", "tTrench"),
    ("enemies_near", "tFoesNear"),
    ("enemies_in_trenches_near", "tFoesTrenched"),
    ("water_on_route_m", "tWater"),
    ("grenade_pickup_m", "tGrenade"),
    ("spray_pickup_m", "tSpray"),
    ("medkit_pickup_m", "tMedkit"),
    ("armor_pickup_m", "tArmor"),
]
...
def candidate_block(k: int) -> str:
    ...
    lines.append("  if useTerrain then")
    lines.append(f"    terrainFacts(candHeart({k}))")
    for name, value in TERRAIN_FIELDS:
        lines.append(f'    oracleState(strNew("candidates[{k}].{name}"), {value})')
    lines.append("  end if")
...
RULES_TERRAIN = (
    "Owned hearts keep scoring with nobody standing on them. The first team to 900 points wins. "
    "A team with no lives left loses on the spot. Hearts are named by index. A heart's distance, "
    "reach time and water on the route are measured from the cog being asked; the other "
    "distances are measured from the heart. Water in the lake slows a cog to a quarter of its "
    "speed and leaves it exposed. Trenches are pits: 70% of gunfire from outside passes over a "
    "cog standing in one, and climbing out is five times slower. A grenade flies over walls and "
    "deals two damage to every cog in its blast, and six to each cog in the trench it lands in, "
    "which kills through full armour; grenades refill five seconds after pickup. A spray can "
    "hits every cog in a short cone for three damage and ignores trench cover, but it replaces "
    "the gun until the carrier dies. A tight enemy group is where one grenade or one spray burst "
    "hurts the most cogs. Medkits, armour and spray refill thirty seconds after pickup. A value "
    "of -1 means none is known."
)
...
if wantSquad or wantSelf then
  ...
  if useTerrain then
    ...
    oracleStateText(strNew("rules"), strNew("%RULES_TERRAIN%"))
  end if
end if
if wantSquad then
  candCount = 0
...
""" \
        ...
        .replace("%RULES_TERRAIN%", RULES_TERRAIN) \
        .replace("%RULES%", RULES) \
...
WEAPONS = """' ---- Grenades where they hurt most. ----
...
def build(base: str) -> str:
    ...
    # The baseline's one walk command, routed around the lake when useDryRoute says so.
    s = swap(s, "\nwalkTo(moveX, moveY)\n", "\n" + DRY_ROUTE)
...
def main() -> None:
    ...
    size = len(out.encode())
    assert size <= 131_072, f"jev.bas is {size} bytes; the source limit is 131,072"
    ...
    print(f"jev.bas: {len(out.splitlines())} lines, {size} bytes (limit 131,072); wrote "
